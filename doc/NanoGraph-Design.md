...
The NanoGraph-Interpreter schedules a linked list of computing nodes interconnected by arcs.  
...
The offset is translated by each NanoGraph interpreter instance into a physical address defined in the Platform Manifest.

The graph is placed in shared memory accessible to all processors.  
There is no message-passing scheme.  
NanoGraph-Interpreter scheduler instances perform the same estimations in parallel and independently decide which node should be executed with priority.
...
```
;--------------------------------------------------------------------------
;   Stream-based processing using a NanoGraph interpreter :                    
;   
...
## NanoGraph: control of the scheduler

The first words of the binary graph specify the portion of the graph that must be moved to RAM.

To ensure address portability between processors, the NanoGraph interpreter manages a list of memory offsets.  
...
It is the responsibility of the platform layers to manage the synchronization kernels. The NanoGraph Interpreter helps the process by defining a specific memory bank for this problem, and defines a HAL to have the same code running on single core and multi cores devices.
...
The graph can be executed in a multiprocessor and multi tasks platform. Those commands allow the NanoGraph interpreter scheduler to skip the nodes not associated to the current processor / architecture and task.
